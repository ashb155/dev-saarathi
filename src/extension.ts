...
async function getWorkspaceContent(): Promise<string> {
  ...
  for (const file of files) {
    ...
    if (file.path.split("/").some((p) => skipFolders.has(p))) {
      continue;
    }
    try {
      ...
      const entry =
        ...
        content +
        "\n```\n";
      if (totalChars + entry.length > 50000) {
        break;
      }
      context += entry;
...
async function recordAudioWithPython(seconds: number): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code: number) => {
      ...
      if (code === 0 && output.trim() === "SILENCE") {
        reject(new Error("SILENCE"));
      } else if (code === 0 && output.trim()) {
        resolve(output.trim());
      } else {
        reject(new Error(error || "Recording failed"));
      }
    });
  });
}

function waitForBrowserAudio(
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider("devSaarathi.chatView", provider),
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("dev-saarathi.start", () => {
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    const updateContext = async () => {
      ...
      const label = isProject
        ? editor
          ? path.basename(editor.document.fileName) + " + project"
          : "Project"
        : editor
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.command === "startRecording") {
        ...
        if (browserAudioReject) {
          browserAudioReject(
            new Error(message.error || "Browser recording failed"),
          );
          browserAudioResolve = null;
          browserAudioReject = null;
        }
...
async function executeAgenticAction(
  intent: string,
  content: string,
  agenticFile?: string,
) {
  ...
  if (intent === "VAANI" && agenticFile) {
    ...
    const msg = fileExists
      ? "✅ Dev-Saarathi updated " + agenticFile
      : "✨ Dev-Saarathi created " + agenticFile;
  ...
  if (intent === "DOSH" && agenticFile) {
    ...
    vscode.window.showInformationMessage(
      "🔧 Dev-Saarathi applied fix to " + agenticFile,
    );
    return;
  }
  ...
  if (agenticFile === "__GIT__") {
    ...
    terminal.show();
    terminal.sendText(
      "# Suggested by Dev-Saarathi — review and press Enter to run:",
    );
    ...
    vscode.window.showInformationMessage(
      "🚀 Dev-Saarathi: Git commands ready in terminal — review and run!",
    );
    return;
  }
  ...
  if (agenticFile) {
    ...
    const root = workspaceFolders[0].uri;
    const isTest =
      agenticFile.startsWith("test_") || agenticFile === "test_suite.py";
    ...
    if (isTest) {
      ...
      cleanContent =
        firstHeading > 0
          ? content.slice(firstHeading + 1).trim()
          : content.trim();
    }
    const fileUri = vscode.Uri.joinPath(root, agenticFile);
...
async function handleRecording(webview: vscode.Webview, seconds: number) {
  ...
  try {
    webview.postMessage({
      command: "status",
      text: "Recording for " + seconds + "s... Speak now!",
    });
    let audioBase64: string;
    try {
      audioBase64 = await recordAudioWithPython(seconds);
    } catch (recErr) {
      if (String(recErr).includes("SILENCE")) {
        throw recErr;
      }
      // Python recording failed (Codespaces, no audio device, etc.)
      // Fall back to browser microphone via webview
      webview.postMessage({
        command: "status",
        text: "🎤 Recording via browser mic for " + seconds + "s...",
      });
      audioBase64 = await waitForBrowserAudio(webview, seconds);
    }
    if (pollingAborted) {
      webview.postMessage({ command: "error", text: "Request cancelled." });
      return;
    }
    ...
    if (workspaceContext) {
      body.code_context = workspaceContext;
    } else if (codeContext) {
      body.code_context = codeContext;
    }
    if (activeEditor) {
      body.active_filename = path.basename(activeEditor.document.fileName);
    }
    // Trim context if payload is too large for Lambda async invoke (256KB limit)
    let bodyStr = JSON.stringify(body);
    if (bodyStr.length > 200000 && body.code_context) {
      body.code_context = body.code_context.substring(
        0,
        Math.max(
          1000,
          body.code_context.length - (bodyStr.length - 200000) - 200,
        ),
      );
      bodyStr = JSON.stringify(body);
    }
    const triggerRaw = await httpsPost(API_BASE + "/voice", bodyStr);
    const triggerData = JSON.parse(triggerRaw) as {
      job_id?: string;
      error?: string;
    };
    if (!triggerData.job_id) {
      webview.postMessage({
        command: "error",
        text: triggerData.error || "Failed to start job",
      });
      return;
    }
    webview.postMessage({
      command: "status",
      text: "Processing with Dev-Saarathi AI...",
    });
    for (let i = 0; i < 60; i++) {
      await sleep(3000);
      if (pollingAborted) {
      ...
      const resultRaw = await httpsGet(
        API_BASE + "/result/" + triggerData.job_id,
      );
      ...
      if (resultData.status === "COMPLETED") {
        ...
        const isAgentic =
          !!(intent === "VAANI" && resultData.agentic_file) ||
        ...
        webview.postMessage({
          command: "error",
          text: resultData.error || "Processing failed",
        });
        return;
      }
    }
    ...
    if (msg.includes("SILENCE")) {
      webview.postMessage({
        command: "error",
        text: "🤫 No audio detected — please speak clearly and try again!",
      });
    } else if (
      /ECONNREFUSED|ENOTFOUND|ETIMEDOUT|EAI_AGAIN|network|socket hang up/i.test(
        msg,
      )
    ) {
      webview.postMessage({
        command: "error",
        text: "🔌 Backend unavailable — check your internet connection and try again.",
      });
    } else {
...
function buildJS(): string {
  return [
    ...
    "}",
    "",
    "// ── Browser Audio Recording (Web Audio API → WAV) ──────────────────────────",
